...
// Fonction utilitaire pour nettoyer le HTML pour les aperçus
...
export default function App() {
  ...
  // Injection Tailwind + Styles pour l'éditeur riche
  useEffect(() => {
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto justify-end">
            <div className="flex bg-slate-100/50 p-1 rounded-lg border border-slate-100">
              ...
              <button onClick={() => setCurrentView('minutes')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${currentView === 'minutes' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>
                <BookOpen size={14} /> C.R.
              </button>
            </div>
...
// --- Composant Comptes Rendus ---
function MeetingMinutesView({ currentUser, userProfile }) {
  const [currentFolder, setCurrentFolder] = useState('team'); // 'team' | 'personal'
  ...
  // 1. Tolérance minuscules/majuscules pour l'email
  const isFlorent = (currentUser?.email?.toLowerCase() === 'florent.lahilla@iziwup.com') || (userProfile?.displayName === 'Florent (Test)');
  
  // 2. Droit de création : Seulement Florent dans le dossier Public
  const canCreate = currentFolder === 'team' ? isFlorent : true;

  // 3. Droit de modif/suppression : Florent OU le créateur du document (ownership)
  const canModify = currentFolder === 'team' ? (isFlorent || (selectedMinute && selectedMinute.createdBy === currentUser.uid)) : true;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedMinute.id === 'new') {
        const docRef = await addDoc(minutesRef, {
          ...data,
          createdBy: currentUser.uid, // Important pour le check ownership
      ...
      <div className="w-full md:w-1/3 bg-white rounded-2xl border border-slate-200 flex flex-col overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-100 flex flex-col gap-4 bg-slate-50">
          <div className="flex justify-between items-center">
            ...
            {canCreate && <button onClick={handleCreate} className="bg-indigo-600 hover:bg-indigo-700 text-white p-2 rounded-lg transition-colors shadow-sm"><Plus size={16} /></button>}
          </div>
        ...
        {!selectedMinute ? (
          ...
          <>
            <div className="p-4 border-b border-slate-100 flex flex-col gap-3 bg-slate-50">
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2 ml-4">
                  {isEditing ? (
                    ...
                    canModify && (
                      <>
                        <button onClick={handleDelete} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={18} /></button>
              ...
              {isEditing && (
                <div className="flex gap-1 pt-1 border-t border-slate-200/50 mt-1">
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('bold'); }} className="p-1.5 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded transition-colors" title="Gras"><Bold size={16} /></button>
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('italic'); }} className="p-1.5 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded transition-colors" title="Italique"><Italic size={16} /></button>
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('underline'); }} className="p-1.5 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded transition-colors" title="Souligné"><Underline size={16} /></button>
                  <div className="w-px bg-slate-300 mx-1 h-4 self-center"></div>
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('insertUnorderedList'); }} className="p-1.5 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded transition-colors" title="Liste à puces"><List size={16} /></button>
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('insertOrderedList'); }} className="p-1.5 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded transition-colors" title="Liste numérotée"><ListOrdered size={16} /></button>
                </div>
              )}
            </div>
...
function TaskModal({ task, onClose, onSave }) {
  ...
  const handleSubmit = () => {
    ...
    onSave(task.id, { text: title, description: contentToSave, deadline, attachments: links });
    onClose();
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full text-lg font-bold text-slate-800 bg-white border border-slate-200 rounded-lg px-3 py-2 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none" placeholder="Nom de la tâche..." />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all bg-white">
                <div className="flex gap-1 p-1.5 bg-slate-50 border-b border-slate-200">
                  ...
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('insertUnorderedList'); }} className="p-1 text-slate-600 hover:bg-white hover:text-indigo-600 rounded transition-colors" title="Liste à puces"><List size={14} /></button>
                  <button onMouseDown={(e) => { e.preventDefault(); execCmd('insertOrderedList'); }} className="p-1 text-slate-600 hover:bg-white hover:text-indigo-600 rounded transition-colors" title="Liste numérotée"><ListOrdered size={14} /></button>
                </div>
                <div
                  ref={editorRef}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              {deadline && <button onClick={handleAddToCalendar} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors border border-indigo-100"><CalendarPlus size={14} /> RDV 9h</button>}
            </div>
          </div>
              ...
              <div className="space-y-2 mb-3">
                {links.map((link, idx) => (
                  <div key={idx} className="flex items-center justify-between bg-indigo-50/50 p-2 rounded-lg border border-indigo-100 group">
                    ...
                    <button onClick={() => removeLink(idx)} className="text-indigo-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all px-2"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            )}
...
function DataManager({ currentUser, viewedUserId, currentView }) {
  ...
  const updateTask = async (taskId, updates) => {
    try { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'team_tasks', taskId), updates); } catch (e) { console.error(e); }
  };
  ...
  return (
    <>
      ...
      {editingTask && <TaskModal task={editingTask} onClose={() => setEditingTask(null)} onSave={updateTask} />}
    </>
  );
}
...
function Quadrant({ config, tasks, onAdd, onDelete, onToggle, onEdit, onMoveTask }) {
  ...
  return (
    <div 
      className={`flex flex-col rounded-2xl border transition-all duration-300 bg-white h-full overflow-hidden shadow-sm hover:shadow-md ${isDragOver ? 'ring-2 ring-indigo-400 bg-indigo-50/10' : config.borderColor}`}
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar bg-white">
        {tasks.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-2 pointer-events-none">
        ...
        {tasks.map(task => (
          ...
          >
            ...
            <div className="absolute right-2 top-2 opacity-0 group-hover:opacity-100 flex gap-1 transition-opacity bg-white/80 backdrop-blur-sm rounded-lg p-0.5 shadow-sm">
               ...
               <button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-md"><Trash2 size={12} /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-3 border-t border-slate-50 bg-white">
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          <div className="flex items-center gap-2 bg-slate-50 rounded-lg px-3 py-2 border border-transparent focus-within:border-indigo-100 focus-within:bg-white focus-within:shadow-sm transition-all">
            ...
            <input type="text" value={newItem} onChange={(e) => setNewItem(e.target.value)} placeholder="Nouvelle tâche..." className="flex-1 text-sm bg-transparent border-none focus:ring-0 placeholder:text-slate-400 outline-none text-slate-700" />
            <button type="button" onClick={() => setShowDateInput(!showDateInput)} className={`p-1.5 rounded-md transition-colors ${showDateInput || newDeadline ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400 hover:text-indigo-600 hover:bg-slate-100'}`} title="Ajouter une date limite"><CalendarIcon size={14} /></button>
          </div>
          {showDateInput && <div className="flex items-center gap-2 px-1 animate-in slide-in-from-top-1 duration-200"><span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Pour le :</span><input type="date" value={newDeadline} onChange={(e) => setNewDeadline(e.target.value)} className="text-xs bg-slate-50 border border-slate-200 rounded px-2 py-1 outline-none focus:border-indigo-400 text-slate-600" /></div>}
        </form>
      </div>
    </div>
  );
...
// --- Login Screen (Google Uniquement + Fallback Test pour Aperçu) ---
function LoginScreen({ onJoin, auth, user }) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // CORRECTION : On affiche le bouton de secours pour TOUTES les erreurs
      setShowTestLogin(true);

      if (err.code === 'auth/operation-not-allowed') {
         setError("Google Auth non activé dans Firebase.");
         ...
         setError("Erreur connexion (Aperçu restreint). Utilisez le mode Test.");
      }
    }
  };
  ...
  const handleTestLogin = async () => {
    try {
      ...
      // Simule une connexion "Florent" pour le test
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4 font-sans selection:bg-indigo-100">
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-4">
          ...
          {/* Bouton de secours uniquement si erreur (pour l'aperçu) */}
          {showTestLogin && (
            <div className="animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="relative flex py-2 items-center">
                <div className="flex-grow border-t border-slate-100"></div>
                <span className="flex-shrink mx-4 text-slate-300 text-[10px] uppercase font-bold tracking-wider">Mode Secours</span>
                <div className="flex-grow border-t border-slate-100"></div>
              </div>
              <button 
                onClick={handleTestLogin} 
                className="w-full flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-600 font-medium py-2 px-4 rounded-xl border border-slate-200 border-dashed transition-all text-xs"
              >
                <TestTube size={14} /> Connexion Test (Aperçu)
              </button>
            </div>
          )}
        </div>
